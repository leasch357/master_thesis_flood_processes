event_classification_CP_method <- function(input_data, model_output,warmup, runoff_peaks, start_date = NULL, end_date = NULL) {
  ...
  #runoff_peaks: dataframe which includes sm values used for min max normalization
  #start_date and end_date: used for classification of specific periods e.g. future periods 
  #output:
  ...
  #extract annual maxima (amax) of observed discharge 
  ...
  #adjust to selected period
  if (!is.null(start_date)) {
    snowpack <- snowpack %>% filter(date >= start_date)
  }
  if (!is.null(end_date)) {
    snowpack <- snowpack %>% filter(date <= end_date)
  }
  ...
  #adjust to selected period
  if (!is.null(start_date)) {
    Rain_df <- Rain_df %>% filter(date >= start_date-7)
  }
  if (!is.null(end_date)) {
    Rain_df <- Rain_df %>% filter(date <= end_date)
  }
  ...
  #compute thresholds used by decision tree
